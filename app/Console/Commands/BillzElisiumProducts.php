...
use App\Product;
use App\Services\BillzElisiumService;
use App\Services\BillzRayyonService;
use Illuminate\Console\Command;
use Illuminate\Support\Str;


class BillzElisiumProducts extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'BillzElisium:products';
     ...
     * @var string
     */
    protected $description = 'Products from Elisium via Billz';
    ...
    {
        $service = new BillzElisiumService();
        $body = [
            "jsonrpc" => "2.0",
            "method" => "products.get",
            "params" => [
        ...
        // Billz
        $percent = BillzPartner::select('percent')->where('code', BillzPartner::BILLZ_ELISIUM)->first();

        if ($dataBillz->getStatusCode() == 200) {
            foreach ($arr->result as $key => $value) {
                $count = Product::where('sku', BillzPartner::BILLZ_ELISIUM.'-'.$value->sku)->count();
                if ($count == 0) {
                    if (!empty($value->imageUrls)) {
                        if ($value->price != 0) {
                            ...
                            $p = Product::create([
                                ...
                                'sku' => BillzPartner::BILLZ_ELISIUM.'-'.$value->sku,
                                'name' => $value->name,
                                'price' => $new_price,
                                'in_stock' => $value->qty,
                    ...
                    if ($value->price != 0) {
                        ...
                        Product::where('sku', BillzPartner::BILLZ_ELISIUM.'-'.$value->sku)->update([
                            'barcode' => $value->barCode,
                            'sku' => BillzPartner::BILLZ_ELISIUM.'-'.$value->sku,
                            'name' => $value->name,
                            'price' => $new_price,
