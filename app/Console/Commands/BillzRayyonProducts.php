...
class BillzRayyonProducts extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'BillzRayyon:products';
     ...
     * @var string
     */
    protected $description = 'Products from Rayyon via Billz';
    ...
    {
        $service = new BillzRayyonService();
        $body = [
            "jsonrpc" => "2.0",
            "method" => "products.get",
            "params" => [
        ...
        // Billz
        $percent = BillzPartner::select('percent')->where('code', BillzPartner::BILLZ_RAYYON)->first();

        if ($dataBillz->getStatusCode() == 200) {
            foreach ($arr->result as $key => $value) {
                $count = Product::where('sku', BillzPartner::BILLZ_RAYYON.'-'.$value->sku)->count();
                if ($count == 0) {
                    if (!empty($value->imageUrls)) {
                        if ($value->price != 0) {
                            ...
                            $p = Product::create([
                                ...
                                'sku' => BillzPartner::BILLZ_RAYYON.'-'.$value->sku,
                                'name' => $value->name,
                                'price' => $new_price,
                                'in_stock' => $value->qty,
                    ...
                    if ($value->price != 0) {
                        ...
                        Product::where('sku', BillzPartner::BILLZ_RAYYON.'-'.$value->sku)->update([
                            'barcode' => $value->barCode,
                            'sku' => BillzPartner::BILLZ_RAYYON.'-'.$value->sku,
                            'name' => $value->name,
                            'price' => $new_price,
