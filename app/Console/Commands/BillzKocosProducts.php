...
use App\Product;
use App\Services\BillzKocosService;
use Illuminate\Console\Command;
use Illuminate\Support\Str;

class BillzKocosProducts extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'BillzKocos:products';
     ...
     * @var string
     */
    protected $description = 'Products from Kocos via Billz';
    ...
    public function handle()
    {
        ...
        // Billz
        $percent = BillzPartner::select('percent')->where('code', BillzPartner::BILLZ_COKOS)->first();

        if ($dataBillz->getStatusCode() == 200) {
            foreach ($arr->result as $key => $value) {
                $count = Product::where('sku', '8888-02-'.$value->sku)->count();
                if ($count == 0) {
                    if (!empty($value->imageUrls)) {
                        if ($value->price != 0) {
                            ...
                            $seo_title = "Купить косметику " . $value->name . " в Ташкенте, цена ";
                            $meta_description = "В интернет-магазине Allgood.uz можно купить косметику" . $value->name . " по доступной цене в каталоге. Заказать с доставкой по Узбекистану ";
                            ...
                            $p = Product::create([
                                ...
                                'sku' => BillzPartner::BILLZ_COKOS.'-'.$value->sku,
                                'name' => $value->name,
                                'price' => $new_price,
                                'in_stock' => $value->qty,
                    ...
                    if ($value->price != 0) {
                        ...
                        Product::where('sku', BillzPartner::BILLZ_COKOS.'-'.$value->sku)->update([
                            'barcode' => $value->barCode,
                            'sku' => BillzPartner::BILLZ_COKOS.'-'.$value->sku,
                            'name' => $value->name,
                            'price' => $new_price,
