...
class BillzAdrasProducts extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'BillzAdras:products';
     ...
     * @var string
     */
    protected $description = 'Products from Adras via Billz';
    ...
    public function handle()
    {
        ...
        // Billz
        $percent = BillzPartner::select('percent')->where('code', BillzPartner::BILLZ_ADRAS)->first();

        if ($dataBillz->getStatusCode() == 200) {
            foreach ($arr->result as $key => $value) {
                $count = Product::where('sku', BillzPartner::BILLZ_ADRAS.'-'.$value->sku)->count();
                if ($count == 0) {
                    if (!empty($value->imageUrls)) {
                        if ($value->price != 0) {
                            ...
                            $p = Product::create([
                                ...
                                'sku' => BillzPartner::BILLZ_ADRAS.'-'.$value->sku,
                                'name' => $value->name,
                                'price' => $new_price,
                                'in_stock' => $value->qty,
                    ...
                    if ($value->price != 0) {
                        ...
                        Product::where('sku', BillzPartner::BILLZ_ADRAS.'-'.$value->sku)->update([
                            'barcode' => $value->barCode,
                            'sku' => BillzPartner::BILLZ_ADRAS.'-'.$value->sku,
                            'name' => $value->name,
                            'price' => $new_price,
