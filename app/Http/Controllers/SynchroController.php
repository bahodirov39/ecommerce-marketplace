    ...
    public function torgsoft()
    {
        ...
        if (($handle = fopen($file, 'r')) !== false) {
            ...
            while (($data = fgetcsv($handle, 0, ";")) !== false) {
                ...
                $active = $data[13] ? (int)$data[13] : Product::STATUS_INACTIVE;
                ...
                if ($product) {
                    // product found update only price, quantity, status
                    $product->in_stock = $quantity;
                    $product->price = $price;
                    $product->sale_price = $sale_price;
                    $product->status = $price == 0 ? Product::STATUS_INACTIVE : (int)$active;
                    $product->save();
                } else {
                    // product not found
