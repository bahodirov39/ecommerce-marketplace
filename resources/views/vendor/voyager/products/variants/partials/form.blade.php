...
<div class="panel-body">
    ...
    <div class="form-group">
        <label for="installment_price">Цена в рассрочку</label>
        <input type="text" name="installment_price" id="installment_price" class="form-control" value="{{ old('installment_price', optional($variant)->installment_price) }}">
    </div>
    ...
    <div class="form-group" >
        ...
        <select name="status" id="status" class="form-control">
            @foreach (\App\ProductVariant::statuses() as $key => $status)
                <option value="{{ $key }}" @if(old('status', optional($variant)->status)) selected @endif>{{ $status }}</option>
            @endforeach
        </select>
    ...
    <div class="form-group" >
        <label class="control-label" for="variant_image">Картинка</label>
        @if ($variant->image)
            <div style="margin-bottom: 15px;">
                <img src="{{ $variant->small_img }}" alt="">
            </div>
        @endif
        <input type="file" name="image" id="variant_image">
    </div>
