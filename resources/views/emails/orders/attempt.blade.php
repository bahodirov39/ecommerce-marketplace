...
<title>Попытка оформления заказа</title>
</head>
...
<td class="content-cell" style="padding: 30px 20px;">
    <h1>
        {{ setting('site.title') }} - Попытка оформления заказа
    </h1>
    <p>
        <strong>{{ __('main.form.name') }}:</strong> {{ $data['name'] ?? '' }} <br />
        <strong>{{ __('main.phone_number') }}:</strong> {{ $data['phone_number'] ?? '' }} <br />
        {{-- <strong>{{ __('main.email') }}:</strong> {{ $data['email'] ?? '' }} <br /> --}}
        <strong>{{ __('main.address') }}:</strong> {{ $data['address'] ?? '' }} <br />
        <strong>{{ __('main.message') }}:</strong> {{ $data['message'] ?? '' }} <br />
        <strong>{{ __('main.payment_method') }}:</strong> {{ Helper::paymentMethodTitle($data['payment_method_id'] ?? '') }} <br />
        @if (!empty($data['alifshop_phone_number'])) <strong>Телефон alif nasiya: </strong> {{ $data['alifshop_phone_number'] ?? '' }} <br /> @endif
    </p>
    <div>
        <h3 style="margin-bottom: 10px;">{{ __('main.products') }}:</h3>
        @foreach($cartItems as $item)
            <p>
                <a href="{{ $item->associatedModel->url }}">
                    {{ $item->quantity }} x {{ $item->name }} - {{ Helper::formatPrice($item->getPriceSumWithConditions()) }}
                </a>
            </p>
        @endforeach
    </div>
